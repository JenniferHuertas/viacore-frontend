  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        const paymentsThisMonth = approvedPayments.filter((p) => {
        ...
        setStats({
          ...
          requests: requests.filter((r) => r.status !== "confirmed").length,

          trainings: requests.filter((r) => r.status === "confirmed").length,

          meetings: meetings.filter((m) => m.status === "Confirmada").length,
        ...
        const upcoming = meetings
          .filter(
            (m) => m.status === "Confirmada" && new Date(m.startTime) >= now,
          )
          .sort(
            (a, b) =>
    ...
    <AdminLayout>
      <div className="space-y-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
          ...
          <StatCard label="Reuniones pendientes" value={stats.meetings} />

          <StatCard
            label="Pagos este mes"
            value={stats.payments}
            sub={`$${stats.paymentsAmount.toLocaleString("es-AR")}`}
          />
        </div>
                  ...
                  >
                    <div>
                      <p className="text-white text-sm font-medium">
                        {m.user?.companyName || m.user?.name || "Usuario"}
                      </p>

                      <p className="text-gray-400 text-xs mt-0.5">
                        {m.trainingRequest?.training?.title || "Capacitación"}
                      </p>
                    </div>

                    <div className="text-right">
                      <p className="text-[#C7962D] text-sm">
                        {new Date(m.startTime).toLocaleDateString("es-AR")}
                      </p>
              ...
              <div className="space-y-3">
                {latestRequests.map((r) => (
                  ...
                  >
                    <div>
                      <p className="text-white text-sm font-medium">
                        {r.user?.companyName || r.user?.name || "Usuario"}
                      </p>

                      <p className="text-gray-400 text-xs mt-0.5">
                        {r.training?.title || "Capacitación"}
                      </p>
                    </div>
...
function StatCard({
  ...
  return (
    <div className="p-6 rounded-2xl bg-[#0B0D0F] border border-white/10 shadow-lg">
      ...
      {sub && <p className="text-[#C7962D] text-sm mt-1">{sub}</p>}
    </div>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-xs px-2 py-1 rounded-lg border ${s.class}`}>
      {s.label}
    </span>
  );
}
